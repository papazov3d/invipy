...
import vtk
import modelview
from inout import vtkio
import vis.vtkpoly
import vis.vtkvol
import random

class DataContainer(modelview.Observable):
  ...
  def __init__(self):
    modelview.Observable.__init__(self)
  ...
  def load_files(self, file_names, file_load_callback = None):
    ...
    vtk_io = vtkio.VtkIO()
    self.__add_data_items(vtk_io.load_files(unique_file_names, file_load_callback))
  ...
  def __create_and_save_model(self, file_name, data):
    ...
    if isinstance(data, vtk.vtkImageData):
      ...
      volume_model = vis.vtkvol.VtkVolumeModel(file_name, self.create_unique_name(file_name), data)
      ...
      poly_model = vis.vtkpoly.VtkPolyModel(file_name, self.create_unique_name(file_name), data)
